...
private async getQuestionWithAnswer(courseId: string, userId: string, questionId: string): Promise<LearnResponseDto> {
  ...
  await this.neo4jService.write(
    `MATCH (u:User {userId: $userId}), (q:Question {questionId: $questionId})
     MERGE (u)-[:ANSWERED]->(q)`,
    { userId, questionId }
  );

  // Determine parent type (Chapter or SubContent)
  ...
  return {
    ...
    // Include parent content for context
    ...(parent && {
      title: parent.title,
      text: parent.content,
      parentType
...
private async getProgressCount(courseId: string, userId: string): Promise<number> {
  const result = await this.neo4jService.read(
    `MATCH (u:User {userId: $userId})-[:FINISHED|ANSWERED]->(item)
